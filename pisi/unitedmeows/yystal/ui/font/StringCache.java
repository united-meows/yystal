...
public class StringCache {
	/**
	 * Reference to the unicode.FontRenderer class. Needed for creating GlyphVectors and retrieving
	 * glyph texture coordinates.
	 */
	private final GlyphCache glyphCache;
	/**
	 * A cache of recently seen strings to their fully layed-out state, complete with color changes and
	 * texture coordinates of all pre-rendered glyph images needed to display this string. The
	 * weakRefCache holds strong references to the Key objects used in this map.
	 */
	private final WeakHashMap<Key, Entry> stringCache = new WeakHashMap<>();
	/**
	 * Every String passed to the public renderString() function is added to this WeakHashMap. As long
	 * as As long as Minecraft continues to hold a strong reference to the String object (i.e. from
	 * TileEntitySign and ChatLine) passed here, the weakRefCache map will continue to hold a strong
	 * reference to the Key object that said strings all map to (multiple strings in weakRefCache can
	 * map to a single Key if those strings only differ by their ASCII digits).
	 */
	private final WeakHashMap<String, Key> weakRefCache = new WeakHashMap<>();
	/**
	 * Temporary Key object re-used for lookups with stringCache.get(). Using a temporary object like
	 * this avoids the overhead of allocating new objects in the critical rendering path. Of course, new
	 * Key objects are always created when adding a mapping to stringCache.
	 */
	private final Key lookupKey = new Key();
	/**
	 * Pre-cached glyphs for the ASCII digits 0-9 (in that order). Used by renderString() to substiture
	 * digit glyphs on the fly as a performance boost. The speed up is most noticable on the F3 screen
	 * which rapidly displays lots of changing numbers. The 4 element array is index by the font style
	 * (combination of Font.PLAIN, Font.BOLD, and Font.ITALIC), and each of the nested elements is index
	 * by the digit value 0-9.
	 */
	GlyphCache.Glyph[][] digitGlyphs = new GlyphCache.Glyph[4][];
	/**
	 * True if digitGlyphs[] has been assigned and cacheString() can begin replacing all digits with '0'
	 * in the string.
	 */
	...
	/**
	 * Reference to the main Minecraft thread that created this GlyphCache object. Starting with
	 * Minecraft 1.3.1, it is possible for GlyphCache.cacheGlyphs() to be invoked from the
	 * TcpReaderThread while processing a chat packet and computing the width of the incoming chat text.
	 * Unfortunately, if cacheGlyphs() makes any OpenGL calls from any thread except the main one, it
	 * will crash LWJGL with a NullPointerException. By remembering the initial thread and comparing it
	 * later against Thread.currentThread(), the StringCache code can avoid calling cacheGlyphs() when
	 * it's not safe to do so.
	 */
	private final Thread mainThread;

	/**
	 * Wraps a String and acts as the key into stringCache. The hashCode() and equals() methods consider
	 * all ASCII digits to be equal when hashing and comparing Key objects together. Therefore, Strings
	 * which only differ in their digits will be all hashed together into the same entry. The
	 * renderString() method will then substitute the correct digit glyph on the fly. This special digit
	 * handling gives a significant speedup on the F3 debug screen.
	 */
	private static class Key {
		/**
		 * A copy of the String which this Key is indexing. A copy is used to avoid creating a strong
		 * reference to the original passed into renderString(). When the original String is no longer
		 * needed by Minecraft, it will be garbage collected and the WeakHashMaps in StringCache will allow
		 * this Key object and its associated Entry object to be garbage collected as well.
		 */
		public String str;

		/**
		 * Computes a hash code on str in the same manner as the String class, except all ASCII digits hash
		 * as '0'
		 *
		...
		public int hashCode() {
			...
			/* True if a section mark character was last seen. In this case, if the next character is a digit,
			 * it must not be considered equal to any other digit. This forces any string that differs in color
		...
		/**
		 * Compare str against another object (specifically, the object's string representation as returned
		 * by toString). All ASCII digits are considered equal by this method, as long as they are at the
		 * same index within the string.
		 *
		 * @return true if the strings are the identical, or only differ in their ASCII digits
		 */
		@Override
		public boolean equals(final Object o) {
			/* There seems to be a timing window inside WeakHashMap itself where a null object can be passed to
			 * this equals() method. Presumably it happens between computing a hash code for the weakly
			 * referenced Key object while it still exists and calling its equals() method after it was garbage
			 * collected. */
			if (o == null) return false;
			/* Calling toString on a String object simply returns itself so no new object allocation is
			 * performed */
	...
	/**
	 * This entry holds the layed out glyph positions for the cached string along with some relevant
	 * metadata.
	 */
	public static class Entry {
		...
		/**
		 * True if the string uses strikethrough or underlines anywhere and needs an extra pass in
		 * renderString()
		 */
		public boolean specialRender;
		/**
		 * True if the string is obfuscated using &k (aka EnumChatFormatting.OBFUSCATED)
		 */
	...
	static class ColorCode implements Comparable<Integer> {
		...
		/**
		 * The index into the stripped string (i.e. with no color codes) of where this color code would have
		 * appeared
		 */
		...
		/**
		 * Performs numeric comparison on stripIndex. Allows binary search on ColorCode arrays in
		 * layoutStyle.
		 *
	...
	/**
	 * A single StringCache object is allocated by Minecraft's FontRenderer which forwards all string
	 * drawing and requests for string width to this class.
	 */
	public StringCache() {
	 ...
	 * string cache is flushed so that all visible strings will be immediately re-layed out using the
	 * new font selection.
	 *
	 ...
	 * and called any time the font selection changes at runtime via setDefaultFont().
	 */
	...
	/**
	 * Add a string to the string cache by perform full layout on it, remembering its glyph positions,
	 * and making sure that every font glyph used by the string is pre-rendering. If this string has
	 * already been cached, then simply return its existing Entry from the cahe. Note that for caching
	 * purposes, this method considers two strings to be identical if they only differ in their ASCII
	 * digits; the renderString() method performs fast glyph substitution based on the actual digits in
	 * the string at the time.
	 *
	 ...
	 */
	public Entry cacheString(final String str) {
		/* New Key object allocated only if the string was not found in the StringCache using lookupKey.
		 * This variable must be outside the (entry == null) code block to have a temporary strong reference
		 * between the time when the Key is added to stringCache and added to weakRefCache. */
		Key key;
		/* Either a newly created Entry object for the string, or the cached Entry if the string is already
		 * in the cache */
		Entry entry = null;
		...
		if (entry == null) {
			...
			/* layoutGlyphVector() requires a char[] so create it here and pass it around to avoid duplication
			 * later on */
			final char[] text = str.toCharArray();
			/* Strip all color codes from the string */
			entry = new Entry();
			final int length = stripColorCodes(entry, str, text);
			/* Layout the entire string, splitting it up by color codes and the Unicode bidirectional
			 * algorithm */
			final List<GlyphCache.Glyph> glyphList = new ArrayList<>();
			...
			/* Sort Glyph array by stringIndex so it can be compared during rendering to the already sorted
			 * ColorCode array. This will apply color codes in the string's logical character order and not the
			...
			for (int glyphIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++) {
				...
				/* Adjust the string index for each glyph to point into the original string with unstripped color
				 * codes. The while loop is necessary to handle multiple consecutive color codes with no visible
			...
			/* Do not actually cache the string when called from other threads because GlyphCache.cacheGlyphs()
			 * will not have been called and the cache entry does not contain any texture data needed for
			 * rendering. */
			if (mainThread == Thread.currentThread()) {
				/* Wrap the string in a Key object (to change how ASCII digits are compared) and cache it along with
		...
		/* Do not access weakRefCache from other threads since it is unsynchronized, and for a newly created
		 * entry, the keyRef is null */
		if (mainThread == Thread.currentThread()) {
			/* Add the String passed into this method to the stringWeakMap so it keeps the Key reference live as
			 * long as the String is in use. If an existing Entry was already found in the stringCache, it's
			 * possible that its Key has already been garbage collected. The code below checks for this to avoid
			 * adding (str, null) entries into weakRefCache. Note that if a new Key object was created, it will
			 * still be live because of the strong reference created by the "key" variable. */
	...
	/**
	 * Remove all color codes from the string by shifting data in the text[] array over so it overwrites
	 * them. The value of each color code and its position (relative to the new stripped text[]) is also
	 * recorded in a separate array. The color codes must be removed for a font's context sensitive
	 * glyph substitution to work (like Arabic letter middle form).
	 *
	 ...
	 * @param text       on input it should be an identical copy of str; on output it will be string
	 *                   with all color codes removed
	 *
	 * @return the length of the new stripped string in text[]; actual text.length will not change
	 *         because the array is not reallocated
	 */
	private int stripColorCodes(final Entry cacheEntry, final String str, final char[] text) {
		...
		byte renderStyle = 0;
		byte colorCode = -1;
		/* Search for section mark characters indicating the start of a color code (but only if followed by
		 * at least one character) */
		while ((next = str.indexOf('\u00A7', start)) != -1 && next + 1 < str.length()) {
			/* Remove the two char color code from text[] by shifting the remaining data in the array over on
			 * top of it. The "start" and "next" variables all contain offsets into the original unmodified
			 * "str" string. The "shift" variable keeps track of how many characters have been sripped so far,
			 * and it's used to compute offsets into the text[] array based on the start/next offsets in the
			 * original string. */
	...
	/**
	 * Split a string into contiguous LTR or RTL sections by applying the Unicode Bidirectional
	 * Algorithm. Calls layoutString() for each contiguous run to perform further analysis.
	 *
	...
	private float layoutBidiString(final List<GlyphCache.Glyph> glyphList, final char[] text,
		...
		if (Bidi.requiresBidi(text, start, limit)) {
			...
			if (bidi.isRightToLeft()) return layoutStyle(glyphList, text, start, limit,
				...
				/* Every GlyphVector must be created on a contiguous run of left-to-right or right-to-left text.
				 * Keep track of the horizontal advance between each run of text, so that the glyphs in each run can
				 * be assigned a position relative to the start of the entire string and not just relative to that
				 * run. */
	...
	private float layoutStyle(final List<GlyphCache.Glyph> glyphList, final char[] text, int start,
		...
		/* If no exact match is found, Arrays.binarySearch() returns (-(insertion point) - 1) where the
		 * insertion point is the index of the first ColorCode with a stripIndex > start. In that case,
		 * colorIndex is adjusted to select the immediately preceding ColorCode whose stripIndex < start. */
		if (colorIndex < 0) {
		...
		while (start < limit) {
			...
			/* Search for the next ColorCode that uses a different fontStyle than the current one. If found, the
			 * stripIndex of that new code is the split point where the string must be split into a separately
			 * styled segment. */
	...
	/**
	 * Given a string that runs contiguously LTR or RTL, break it up into individual segments based on
	 * which fonts can render which characters in the string. Calls layoutFont() for each portion of the
	 * string that can be layed out with a single font.
	 *
	 * @param glyphList   will hold all new Glyph objects allocated by layoutFont()
	 * @param text        the string to lay out
	 * @param start       the offset into text at which to start the layout
	 ...
	 * @param advance     the horizontal advance (i.e. X position) returned by previous call to
	 *                    layoutString()
	 * @param style       combination of Font.PLAIN, Font.BOLD, and Font.ITALIC to select a fonts with
	 *                    some specific style
	 *
	 * @return the advance (horizontal distance) of this string plus the advance passed in as an
	...
	private float layoutString(final List<GlyphCache.Glyph> glyphList, final char[] text, int start,
		 ...
		 * Checking the digitGlyphsReady flag prevents a chicken-and-egg problem where the digit glyphs have
		 * to be initially cached and the digitGlyphs[] array initialized without replacing every digit with
		 * '0'. */
		if (digitGlyphsReady) {
		...
		while (start < limit) {
			...
			/* canDisplayUpTo() returns start if the starting character is not supported at all. In that case,
			 * draw just the one unsupported character (which will use the font's "missing glyph code"), then
	...
	/**
	 * Allocate new Glyph objects and add them to the glyph list. This sequence of Glyphs represents a
	 * portion of the string where all glyphs run contiguously in either LTR or RTL and come from the
	 * same physical/logical font.
	 *
	...
	private float layoutFont(final List<GlyphCache.Glyph> glyphList, final char[] text,
		 ...
		 * since it may be incomplete if lookupGlyph() returns null for any glyphs not yet stored in the
		 * glyph cache. */
		if (mainThread == Thread.currentThread()) {
		 ...
		 * object to correctly index the original unstripped string. */
		GlyphCache.Glyph glyph = null;
		final int numGlyphs = vector.getNumGlyphs();
		for (int index = 0; index < numGlyphs; index++) {
			final Point position = vector.getGlyphPixelBounds(index, null, advance, 0).getLocation();
			/* Compute horizontal advance for the previous glyph based on this glyph's position */
			if (glyph != null) {
				glyph.advance = position.x - glyph.x;
			}
			/* Allocate a new glyph object and add to the glyphList. The glyph.stringIndex here is really like
			 * stripIndex but it will be corrected later to account for the color codes that have been stripped
			 * out. */
		...
		/* Compute the advance position of the last glyph (or only glyph) since it can't be done by the
		 * above loop */
