...
public class TTFFontRenderer {
	...
	private void renderString(String text, float x, float y, int color, boolean shadow) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (character == COLOR_INVOKER && i < length) {
				...
				if (strikethrough)
					drawLine(new Vector2f(0f, charData.height / 2f), new Vector2f(charData.width, charData.height / 2f), 3);

				// Draws the underline if enabled.
				if (underlined)
					drawLine(new Vector2f(0f, charData.height - 15), new Vector2f(charData.width, charData.height - 15), 3);
	...
	private void drawLine(Vector2f start, Vector2f end, float width) {
		...
		{
			GL11.glVertex2f(start.first(), start.second());
			GL11.glVertex2f(end.first(), end.second());
		}
